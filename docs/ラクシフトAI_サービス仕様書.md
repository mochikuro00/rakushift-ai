...
| 階層 | 種類 | 緩和可否 | 詳細 |
|------|------|---------|------|
| Tier 1 | 労基法（32条/34条/35条） | 不可 | 労働時間上限、休憩、休日 |
| Tier 2 | 店舗ルール（人員/管理者/定休日） | 条件付き可 | 最低人員、管理者常駐 |
| Tier 3 | 高度な制約（ポジション/人間関係） | 可 | ポジション別人数、最低出勤日数（ソフトペナルティ）、NGペア、必須ペア |
| Tier 4 | 最適化（コスト/均等/バランス） | 可 | シフト回数の均等化、人件費最小化、本人の希望時間優先（トグルON時のみ） |

### 3.3 フォールバック（実践的フェイルセーフ）
```
Tier 1+2+3+4 → 解あり → 最適シフト
             → 解なし → Tier 1+2+3 再挑戦
                      → 解なし → 貪欲法（Greedy）
                           ↓
...
| 申請リスト | 休み希望の承認/却下 |
| 分析・レポート | 月間統計、AI診断 |
| スタッフ管理 | CRUD、評価、勤務条件（最低出勤日数等）、希望時間考慮トグル、担当ポジション、人間関係（NG/必須ペア） |
