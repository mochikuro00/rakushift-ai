    ...
    def _solve_optimized(self):
        try:
            ...
            for s in self.staff_list:
                ...
                print("  Staff '{}' options on {}: {}".format(s.get('name', ''), d0, opt_list))
                ...
                for slot_min, req_count in slot_reqs.items():
                    ...
                    if workers:
                        slack = pulp.LpVariable(
                            f"slot_{d}_{slot_min}", 0, None, pulp.LpInteger
                        )
                        problem += pulp.lpSum(workers) + slack >= req_count
                        penalty += slack * 1000000
                    else:
                        time_str = self._from_minutes(slot_min)
                        print("  CRITICAL: No staff can cover {} {} (need {})".format(d, time_str, req_count))
            ...
            for d in self.dates:
                ...
                for slot_min in slot_reqs.keys():
                    ...
                    if mgr_vars:
                        slack = pulp.LpVariable(
                            f"mgr_{d}_{slot_min}", 0, None, pulp.LpInteger
                        )
        ...
        for d in self.dates:
            ...
            for slot_min, req_count in slot_reqs.items():
                ...
                if coverage < req_count:
                    time_str = self._from_minutes(slot_min)
                    print("  WARNING: {} {} - Need {}, got {}".format(d, time_str, req_count, coverage))
                    violations += 1
