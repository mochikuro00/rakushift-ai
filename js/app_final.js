const app = {
     ...
     */
    async init() {
        console.log("App initializing (Final Version)...");
        
        // 【重要】強制リセット処理
        // 過去のキャッシュや古いIDが残っているとDBと不整合を起こすため、
        // このバージョンが初めて実行される際は必ずLocalStorageを全消去する。
        const resetKey = 'RAKUSHIFT_RESET_V1';
        if (!localStorage.getItem(resetKey)) {
            console.warn("Performing HARD RESET of local storage to fix ID conflicts.");
            localStorage.clear();
        ...
        try {
            ...
            if (API.session) {
                ...
                // ローディング表示を消すために手動で呼び出す
                // (通常はloadDataのfinallyで消えるが、呼ばない場合はここで消す)
            ...
            document.getElementById('viewContainer').innerHTML = `
                <div class="flex flex-col items-center justify-center h-full text-red-500">
                    ...
                    <h2 class="text-xl font-bold mb-2">起動エラー</h2>
                    <p class="text-gray-600 mb-4">アプリケーションの起動に失敗しました。<br>${e.message}</p>
                    <button onclick="location.reload()" class="px-4 py-2 bg-blue-600 text-white rounded-lg font-bold">再読み込み</button>
                </div>
            `;
        } finally {
            this.updateAuthUI();
    ...
    async loadData() {
        ...
        try {
            // 現在のセッションのユーザー情報を確認
            const userEmail = API.session?.user?.email;
            
            // 組織IDの特定（localStorage優先 -> state -> API）
            let orgId = localStorage.getItem('rakushift_org_id') || this.state.organization_id;
            ...
            if (!orgId) {
                try {
                    ...
                    if (orgRes && orgRes.data && orgRes.data.length > 0) {
                        ...
                        console.log("Organization ID loaded from API:", orgId);
                    }
                } catch(e) {
            ...
            // 【修正】組織IDによるフィルタリングを一旦解除
            // RLS(Row Level Security)が効いていれば、自分のデータだけが返るはず。
            // ID指定が間違っていてデータが空になるリスクを避けるため、パラメータなしでリクエストする。
            const queryParams = {}; 
            ...
            if (configRes && configRes.data && configRes.data.length > 0) {
                ...
                // コンフィグから組織IDが取れるならそれも正とする
                if (configRes.data[0].organization_id) {
                    orgId = configRes.data[0].organization_id;
                    this.state.organization_id = orgId;
                    localStorage.setItem('rakushift_org_id', orgId);
                }
            } else {
                // コンフィグがない場合はデフォルトを使うが、既存のconfigがあるなら維持する
                if (Object.keys(this.state.config).length === 0) {
                    this.state.config = { ...this.state.defaultConfig };
                }
                if (orgId) this.state.config.organization_id = orgId;
            }

            // 【重要】データ保護ロジック
            // RLSの反映遅延や一時的なAPIエラーでデータが0件で返ってきた場合、
            // 画面上のデータをクリアしてしまう事故を防ぐため、
            // 「APIがデータを返した場合のみ上書き」し、「空なら既存データを維持」する挙動に変更。
            
            if (staffRes.data && staffRes.data.length > 0) {
                this.state.staff = staffRes.data;
            } else if (this.state.staff.length > 0) {
                console.warn("API returned 0 staff. Keeping local data to prevent disappearance.");
            }

            if (shiftsRes.data && shiftsRes.data.length > 0) {
                this.state.shifts = shiftsRes.data;
            } else if (this.state.shifts.length > 0) {
                console.warn("API returned 0 shifts. Keeping local data to prevent disappearance.");
            }
    ...
    switchLoginTab(tab) {
        ...
        const staffBtn = document.getElementById('tabStaff');
        const adminForm = document.getElementById('formAdmin');
        const staffForm = document.getElementById('formStaff');

        if (tab === 'admin') {
            adminBtn.classList.add('border-blue-600', 'text-blue-600');
            adminBtn.classList.remove('border-transparent', 'text-gray-400');
            staffBtn.classList.add('border-transparent', 'text-gray-400');
            staffBtn.classList.remove('border-blue-600', 'text-blue-600');
            
            adminForm.classList.remove('hidden');
            staffForm.classList.add('hidden');
        } else {
            staffBtn.classList.add('border-blue-600', 'text-blue-600');
            staffBtn.classList.remove('border-transparent', 'text-gray-400');
            adminBtn.classList.add('border-transparent', 'text-gray-400');
            adminBtn.classList.remove('border-blue-600', 'text-blue-600');
    ...
    async login() {
        ...
        try {
            ...
            console.error(e);
            this.showToast('ログイン失敗: ' + (e.message || '不明なエラー'), 'error');
        } finally {
            this.showLoading(false);
        }
    },
    ...
    async signUpMode() {
        ...
        if (!email || !pass) {
            this.showToast('登録するメールアドレスとパスワードを入力してください', 'error');
            return;
        }

        if (!confirm(`以下のメールアドレスで新規登録しますか？\n\n${email}`)) return;

        this.showLoading(true);
        try {
            await API.signUp(email, pass);
            this.showToast('確認メールを送信しました。メール内のリンクをクリックして登録を完了してください。', 'success');
        } catch (e) {
            console.error(e);
            this.showToast('登録失敗: ' + (e.message || '不明なエラー'), 'error');
        } finally {
            this.showLoading(false);
        }
    },
    ...
    updateAuthUI() {
        ...
        const headerContainer = document.getElementById('adminHeaderControls');
        ...
        if (this.state.isAdmin) {
            ...
            // =========================================================
            if(btn) {
                btn.innerHTML = '<i class="fa-solid fa-arrow-right-from-bracket w-6 text-center"></i> ログアウト';
                btn.className = 'flex items-center w-full px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 rounded-lg transition-colors';
            }
            adminLinks.forEach(el => el.classList.remove('hidden'));

            // ヘッダーコントロールを生成 (人件費などはここで初めてDOMに出現)
            if(headerContainer) {
                // 開発者用メニュー判定 (特定のメアドの場合のみ表示)
                const devMenu = (API.session?.user?.email === 'master@mochikuro.com') ? `
                    <button onclick="app.devCreateTestData()" class="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded-lg text-xs font-bold shadow-sm whitespace-nowrap ml-2">
                        <i class="fa-solid fa-database"></i> テストデータ作成
                    </button>
                ` : '';

                headerContainer.innerHTML = `
                    <button id="autoFillBtn" class="flex items-center gap-2 bg-gray-800 hover:bg-gray-700 text-white px-3 py-2 rounded-lg text-xs font-bold shadow-sm transition-all transform active:scale-95 whitespace-nowrap">
                        <i class="fa-solid fa-wand-magic-sparkles text-yellow-400"></i> AI自動作成
                    </button>
                    <button id="aiAdviceBtn" class="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-3 py-2 rounded-lg text-xs font-bold shadow-md shadow-blue-200 transition-all transform active:scale-95 whitespace-nowrap">
                        <i class="fa-solid fa-robot"></i> <span class="hidden md:inline">AI診断</span>
                    </button>
                    ${devMenu}
                `;
                
                // 動的生成したボタンにイベントを付与
                document.getElementById('autoFillBtn').addEventListener('click', () => this.openModal('autoFillModal'));
                document.getElementById('aiAdviceBtn').addEventListener('click', () => this.runAIDiagnosis());
            ...
            // =========================================================
            // スタッフ(ゲスト)モード: 管理者要素をDOMから完全削除
            // =========================================================
            if(btn) {
                btn.innerHTML = '<i class="fa-solid fa-right-to-bracket w-6 text-center"></i> 管理者ログイン';
                btn.className = 'flex items-center w-full px-3 py-2 text-sm font-medium text-blue-600 hover:bg-blue-50 rounded-lg transition-colors';
            }
            adminLinks.forEach(el => el.classList.add('hidden'));
            
            // ヘッダーの中身を空にする（人件費情報などを物理的に消去）
            if(headerContainer) {
                headerContainer.innerHTML = '';
            }
        }
    ...
    async devCreateTestData() {
        ...
        try {
            // 2. 組織IDの確保
            let orgId = this.state.organization_id;
            ...
            if (orgId) {
                ...
                throw new Error("組織IDの取得に失敗しました");
            }
            ...
            if (currentCount < targetCount) {
                ...
                const templates = [
                    { name: "【万能】佐藤 (店長)", role: 'manager', max_days: 7, max_hours: 12, wage: 1500 }, // 制約ほぼなし
                    { name: "【万能】鈴木 (副店長)", role: 'manager', max_days: 6, max_hours: 10, wage: 1400 },
                    { name: "高橋 (リーダー)", role: 'leader', max_days: 5, max_hours: 8, wage: 1300 },
                    { name: "田中 (フル)", role: 'staff', max_days: 5, max_hours: 8, wage: 1100 },
                    { name: "渡辺 (フル)", role: 'staff', max_days: 5, max_hours: 8, wage: 1100 },
                    { name: "フリーターA (長時間)", role: 'staff', max_days: 6, max_hours: 10, wage: 1200 }, // 穴埋め要員
                    { name: "フリーターB (長時間)", role: 'staff', max_days: 6, max_hours: 10, wage: 1200 },
                    { name: "学生C (夕方)", role: 'staff', max_days: 4, max_hours: 5, wage: 1000 },
                    { name: "学生D (夕方)", role: 'staff', max_days: 4, max_hours: 5, wage: 1000 },
                    { name: "主婦E (昼)", role: 'staff', max_days: 4, max_hours: 6, wage: 1050 },
                    { name: "主婦F (昼)", role: 'staff', max_days: 4, max_hours: 6, wage: 1050 },
                    { name: "週末G (土日)", role: 'staff', max_days: 2, max_hours: 8, wage: 1100 },
                    { name: "新人H", role: 'staff', max_days: 3, max_hours: 4, wage: 950 }
                ];

                const evals = ['A', 'A', 'B', 'B', 'C'];
                const getRandomEval = () => evals[Math.floor(Math.random() * evals.length)];
                ...
                for (let i = 0; i < addCount; i++) {
                    ...
                    const res = await API.create('staff', {
                        name: uniqueName,
                        role: tmpl.role,
                        evaluation: getRandomEval(),
                        salary_type: 'hourly',
                ...
                // 念のためリロードして、本当に保存されたか確認することを推奨
                if(confirm("保存に成功した可能性があります。\n念のためリロードして、データが残っているか確認しますか？")) {
                    location.reload();
                }
    ...
    renderCalendar(container) {
        ...
        for (let day = 1; day <= lastDay.getDate(); day++) {
            ...
            html += `
                <div class="bg-white calendar-cell p-1.5 flex flex-col gap-1 relative group min-h-[140px] transition-colors ${hoverClass} ${dateBgClass}" 
                     ${cellAction}>
                    <div class="flex justify-between items-start px-1">
                        <div class="flex flex-col">
                            ...
                            ${isSpecialHoliday ? `<span class="text-[10px] font-bold text-red-600 border border-red-200 bg-white px-1 rounded truncate max-w-[60px]">臨時休業</span>` : ''}
                            ${specialDayConfig ? `<span class="text-[10px] font-bold text-yellow-700 bg-yellow-100 px-1 rounded truncate max-w-[60px]" title="${specialDayConfig.start}-${specialDayConfig.end}">${specialDayConfig.note || '時間変更'}</span>` : ''}
                        </div>
                        ${actionBtns}
                    </div>
                    
                    ${note ? `
                        <div class="mx-1 mt-1 p-1.5 bg-yellow-50 border border-yellow-200 rounded text-[10px] text-yellow-800 leading-tight cursor-pointer hover:bg-yellow-100 transition-colors"
                             onclick="event.stopPropagation(); app.openCalendarNoteModal('${dateStr}')" title="${note}">
                            <i class="fa-regular fa-note-sticky mr-1"></i>${note}
                        </div>
                    ` : ''}
                    ...
                    <div class="flex-1 flex flex-col gap-1 mt-1 overflow-y-auto custom-scrollbar">
                        ${dayShifts.map(shift => {
                            ...
                            if (!staff) return '';
                            const startH = parseInt(shift.start_time);
                            let barColor = 'border-blue-500 bg-blue-50 text-blue-700';
                            if (startH < 10) barColor = 'border-yellow-500 bg-yellow-50 text-yellow-700';
                            if (startH >= 17) barColor = 'border-purple-500 bg-purple-50 text-purple-700';
                            
                            if (isPast) {
                                barColor = 'border-gray-300 bg-gray-200 text-gray-500';
                            }

                            // Admin: Edit. Guest: No action.
                            const shiftAction = this.state.isAdmin ? `onclick="event.stopPropagation(); app.openEditShift('${shift.id}')"` : '';
                            const cursorClass = this.state.isAdmin ? 'cursor-pointer hover:brightness-95' : '';

                            return `
                                <div class="text-[10px] px-2 py-1 rounded border-l-2 shadow-sm transition-all truncate ${barColor} ${cursorClass}" 
                                     ${shiftAction}>
                                    <div class="font-bold">${staff.name}</div>
                                    <div class="opacity-80 font-mono">${shift.start_time}-${shift.end_time}</div>
                                </div>
                            `;
                        }).join('')}
                    </div>
                        ...
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ${this.state.staff.map(s => {
                                    const role = this.state.config.roles.find(r => r.id === s.role) || { name: '未設定', color: 'gray' };
                                    const colorMap = {
                                ...
                                <tr class="hover:bg-gray-50 group transition-colors">
                                    ...
                                    <td class="p-4 whitespace-nowrap">
                                        ...
                                        ${s.evaluation === 'B' || !s.evaluation ? '<span class="bg-blue-50 text-blue-800 text-xs font-bold px-2 py-1 rounded-md border border-blue-100 shadow-sm">B</span>' : ''}
    ...
    async saveSettings() {
        ...
        this.showLoading(true);
        try {
            if (newConfig.id) await API.update('config', newConfig.id, newConfig);
            else { 
                newConfig.organization_id = this.state.organization_id;
                const res = await API.create('config', newConfig); 
                newConfig.id = res.id; 
            }
    ...
    async saveStaff() {
        ...
        // 新規作成時は組織IDを付与
        if (!id) {
            data.organization_id = this.state.organization_id || localStorage.getItem('rakushift_org_id');
        }
    ...
    async runAutoFill() {
        ...
        try {
            ...
            if (targetType === 'reset_all') {
                // Cloud Run側で削除はできないので、クライアントから削除APIを叩く
                // ※本来は一括削除APIが欲しいが、ループで消す
                const shiftsToDelete = this.state.shifts.filter(s => dates.includes(s.date) && new Date(s.date) >= today);
                await Promise.all(shiftsToDelete.map(s => API.delete('shifts', s.id)));
            }

            // Cloud Run 計算実行
            this.showToast('AI計算サーバーにリクエスト送信中...', 'info');
            console.log("Sending request to Cloud Run:", CALC_API_URL); // Debug log
            ...
            if (result.mode === 'diagnostic') {
                ...
                console.log("Staff Count Received:", result.received_data?.staff_count);
                console.log("Dates Count Received:", result.received_data?.date_count);
                console.log("Config Keys:", result.received_data?.config_sample);
                alert(`診断モード完了。\nサーバー受信数: スタッフ${result.received_data?.staff_count}名, 日付${result.received_data?.date_count}日`);
            }
            ...
            if (result.status === 'success' && result.shifts) {
                ...
                for (const s of newShifts) {
                    // 同一日・同一スタッフの既存シフトがあればスキップ(empty_onlyの場合など)
                    // reset_allの場合は消しているので気にしなくていい
                ...
                // バッチ保存 (バックグラウンドで実行)
                const batchSize = 10;
                // 非同期で保存処理を開始し、完了を待たずにUIを更新する
                (async () => {
                    try {
                        for (let i = 0; i < finalShifts.length; i += batchSize) {
                            await Promise.all(finalShifts.slice(i, i + batchSize).map(s => {
                                s.organization_id = this.state.organization_id;
                                return API.create('shifts', s);
                            }));
                        }
                ...
                // クライアント側のStateを即時更新して描画
                // ※サーバー保存の完了を待つと、RLSの反映ラグやネットワーク遅延で「消えた」ように見えるため
                //   生成されたデータを信頼して即時表示する
                
                // 既存のシフトから、今回生成した日付範囲のものを削除(重複防止)
                // ただし empty_only の場合は既存を残す
                if (targetType === 'reset_all') {
                    this.state.shifts = this.state.shifts.filter(s => !dates.includes(s.date));
                }
                
                // 新しいシフトを追加
                // API.createの戻り値を待たず、生成されたオブジェクトに仮IDを振って表示
                const displayShifts = finalShifts.map((s, idx) => ({
                    ...s,
                    id: s.id || `temp_${Date.now()}_${idx}`, // 仮ID
                    organization_id: this.state.organization_id
                }));
    ...
    runAIDiagnosis() {
        ...
        setTimeout(() => {
            const suggestions = [];
            suggestions.push({ type: 'warning', title: '週末の人員不足', desc: '土曜日のランチタイムの人員が不足気味です。', action: '募集強化' });
            content.innerHTML = suggestions.map(s => `
                <div class="bg-white border ${s.type === 'warning' ? 'border-red-200 bg-red-50' : 'border-blue-200 bg-blue-50'} rounded-lg p-4 flex gap-4">
                    ...
                    <div><h4 class="font-bold text-gray-800 mb-1">${s.title}</h4><p class="text-sm text-gray-600 mb-3">${s.desc}</p><button class="text-xs font-bold px-3 py-1.5 bg-white border border-gray-300 rounded shadow-sm hover:bg-gray-50">${s.action}</button></div>
                </div>`).join('');
        }, 1500);
    },
