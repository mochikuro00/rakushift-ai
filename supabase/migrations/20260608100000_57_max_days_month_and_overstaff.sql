-- 57_max_days_month_and_overstaff.sql
-- ===========================================================
-- v3.7.91:
--   1) staff.max_days_month INTEGER (1日〜31日, default 31)
--      → 月の最大出勤日数を個別に管理
--   2) config.allow_overstaffing BOOLEAN (default false)
--      → 過剰配置を許容するか (false=COVERAGE_OVER 100M / true=緩和)
--   3) upsert_staff_by_contract / update_config_by_contract を更新
-- ===========================================================

ALTER TABLE staff
    ADD COLUMN IF NOT EXISTS max_days_month INTEGER DEFAULT 31;

COMMENT ON COLUMN staff.max_days_month IS
    '月の最大出勤日数 (1〜31)。scheduler でハード制約として扱う。';

ALTER TABLE config
    ADD COLUMN IF NOT EXISTS allow_overstaffing BOOLEAN DEFAULT false;

COMMENT ON COLUMN config.allow_overstaffing IS
    '過剰配置を許容するか。false=必要人数ぴったり (COVERAGE_OVER 100M) / true=緩和 (1M)';

-- upsert_staff_by_contract に max_days_month を組み込む
