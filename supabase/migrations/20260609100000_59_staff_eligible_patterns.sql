-- 59_staff_eligible_patterns.sql
-- ===========================================================
-- v3.7.109: スタッフごとに「該当するシフトパターン」をチェックで
--   指定できるようにする。staff.eligible_patterns JSONB 配列
--   例: ["早番", "日勤"] → このスタッフは早番か日勤にのみ入る
--   null or 空配列 → 全パターン該当 (デフォルト)
-- ===========================================================

ALTER TABLE staff
    ADD COLUMN IF NOT EXISTS eligible_patterns JSONB DEFAULT '[]'::jsonb;

COMMENT ON COLUMN staff.eligible_patterns IS
    '該当シフトパターン名の配列。空 or null は全パターン該当 (デフォルト)。';

-- upsert_staff_by_contract を eligible_patterns 対応版で再定義
