-- 61_staff_max_consecutive.sql
-- ===========================================================
-- v3.7.113: 勤務制約に「連続出勤日数 (最大)」を追加。
--   staff.max_consecutive_days INTEGER (デフォルト 6, 1-7 範囲)
--   scheduler は労基法35条デフォルト 6 を上書きしてスタッフ別に
--   ハード制約として扱う。
-- ===========================================================

ALTER TABLE staff
    ADD COLUMN IF NOT EXISTS max_consecutive_days INTEGER DEFAULT 6;

COMMENT ON COLUMN staff.max_consecutive_days IS
    '連続出勤日数の上限 (1〜7)。デフォルト 6 (労基法35条)。例: 3 → 3日連続まで';

-- upsert_staff_by_contract を max_consecutive_days 対応版で再定義
...
BEGIN
    ...
    IF p_data ? 'unavailable_dates' THEN
        v_clean_dates := COALESCE(
            ARRAY(
                SELECT d FROM jsonb_array_elements_text(p_data->'unavailable_dates') AS d
                WHERE trim(d) ~ '^\d{4}-\d{2}-\d{2}$' AND _is_valid_iso_date(trim(d))
            ),
            '{}'::TEXT[]
        );
    END IF;
    ...
    IF p_staff_id IS NOT NULL THEN
        UPDATE staff SET
            ...
            max_consecutive_days = CASE WHEN p_data ? 'max_consecutive_days' THEN (p_data->>'max_consecutive_days')::INTEGER ELSE max_consecutive_days END,
            ...
            ng_weekdays      = CASE WHEN p_data ? 'ng_weekdays'      THEN ARRAY(SELECT jsonb_array_elements_text(p_data->'ng_weekdays')::INTEGER) ELSE ng_weekdays END,
            unavailable_dates = CASE WHEN p_data ? 'unavailable_dates' THEN v_clean_dates ELSE unavailable_dates END,
            pattern_target_counts = CASE WHEN p_data ? 'pattern_target_counts' THEN p_data->'pattern_target_counts' ELSE pattern_target_counts END,
            eligible_patterns = CASE WHEN p_data ? 'eligible_patterns' THEN p_data->'eligible_patterns' ELSE eligible_patterns END,
            ng_holiday = CASE WHEN p_data ? 'ng_holiday' THEN (p_data->>'ng_holiday')::BOOLEAN ELSE ng_holiday END
    ...
    ELSE
        INSERT INTO staff (
            ...
            COALESCE((p_data->>'max_consecutive_days')::INTEGER, 6),
            COALESCE(p_data->>'shift_priority', 'medium'),
            COALESCE(p_data->>'contract_type', 'general'),
            p_data->>'pref_start_wd', p_data->>'pref_end_wd',
            p_data->>'pref_start_we', p_data->>'pref_end_we',
            p_data->>'ng_pairs', p_data->>'req_pairs',
            COALESCE(p_data->>'position', 'any'),
            CASE WHEN p_data ? 'ng_weekdays' THEN ARRAY(SELECT jsonb_array_elements_text(p_data->'ng_weekdays')::INTEGER) ELSE '{}'::INTEGER[] END,
